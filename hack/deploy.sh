...
# deploy.sh [/path/to/manifest.yaml.template]
#
...
keydir="$(mktemp -d)"
manifest="${1:-${basedir}/deployment.yaml.template}"
...
kubectl create namespace webhook-demo || true

# Create the TLS secret for the generated keys.
kubectl -n webhook-demo create secret tls webhook-server-tls \
    --cert "${keydir}/webhook-server-tls.crt" \
    --key "${keydir}/webhook-server-tls.key" || \
    kubectl -n webhook-demo replace secret tls webhook-server-tls \
    --cert "${keydir}/webhook-server-tls.crt" \
    --key "${keydir}/webhook-server-tls.key"
